...
export function tokens(mode: AppTheme | 'light' | 'dark') {
    return mode === AppTheme.LIGHT ? lightTheme : darkTheme
}

// mui theme settings
export function themeSettings(mode: AppTheme) {
...
export function useMode(): [Theme, ColorMode] {
    const [mode, setMode] = useState<AppTheme>(AppTheme.DARK)

    // get an object that contains a function to toggle the color mode
    ...
    // get the mui theme object based on the color mode
